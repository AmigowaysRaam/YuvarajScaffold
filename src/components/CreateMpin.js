...
export default function CreateMpin({ route }) {
  ...
  const handleCreateMpin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetchData("app-employee-login-create-mpin", "POST", {
        user_id: userData?.id,
    ...
    >
      ...
      >
        <View style={styles.container}>
          ...
          <Text style={styles.title}>{t("Secure Access with MPIN")}</Text>
          <Text style={styles.subtitle}>
            {t("Create a 4-digit MPIN to secure your KAS Jewellery account.")}
          </Text>
